    ...
    def play_turn(self):
        ...
        if isinstance(current_player, AIPlayer):
            ...
            if action == "tap":
                ...
                print(
                    f"{current_player.name} taps with hand {my_hand + 1} on opponent's hand {their_hand + 1}."
                )
                ...
                print(
                    f"{current_player.name} splits {points} points from hand {hand_from + 1} to hand {hand_to + 1}."
                )
            ...
            while not valid_move:
                try:
                    ...
                    if action not in ["tap", "split"]:
                        raise ValueError(
                            "Invalid action. Please enter 'tap' or 'split'."
                        )

                    if action == "tap":
                        my_active_hands = [
                            i for i in range(2) if current_player.hands[i] > 0
                        ]

                        if len(my_active_hands) == 1:
                            my_hand = my_active_hands[0]
                            print(
                                f"You only have one active hand. Using hand {my_hand + 1} automatically."
                            )
                        else:
                            my_hand = (
                                int(input(f"Which of your hands (1 or 2)? ")) - 1
                        ...
                        if len(active_hands) == 1:
                            ...
                            print(
                                f"Opponent only has one hand active. Tapping hand {their_hand + 1}."
                            )
                        else:
                            their_hand = (
                                int(input(f"Which opponent hand to tap (1 or 2)? ")) - 1
                            )  # Subtract 1 for 0-based index
                        ...
                        hand_from = (
                            int(input(f"Which hand to take points from (1 or 2)? ")) - 1
                        )  # Subtract 1 for 0-based index
                        ...
                        if current_player.hands[hand_from] == 1:
                            print(
                                f"Hand {hand_from + 1} only has 1 point. Automatically moving it to hand {hand_to + 1}."
                            )
                            points = 1
                        else:
